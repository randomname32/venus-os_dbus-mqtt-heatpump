...
driver_name="dbus-mqtt-heatpump"
...
# latest release
latest_release_stable=$(curl -s https://api.github.com/repos/randomname32/venus-os_${driver_name}/releases/latest | grep "tag_name" | cut -d : -f 2,3 | tr -d "\ " | tr -d \" | tr -d \,)

# nightly build
latest_release_nightly=$(curl -s https://raw.githubusercontent.com/randomname32/venus-os_${driver_name}/master/${driver_name}/${driver_name}.py | grep FirmwareVersion | awk -F'"' '{print $4}')
...
while true; do
    ...
    if [[ -z "$driver_instance" || ( "$driver_instance" =~ ^[0-9]+$ && "$driver_instance" -ge 1 && "$driver_instance" -le 99 ) ]]; then
        ...
        echo "Invalid input. Please enter a number between 1 and 99 or press enter."
    fi
done
...
if [ "$version" = "latest release \"$latest_release_stable\"" ]; then
    url=$(curl -s https://api.github.com/repos/randomname32/venus-os_${driver_name}/releases/latest | grep "zipball_url" | sed -n 's/.*"zipball_url": "\([^"]*\)".*/\1/p')
fi

## nightly build
if [ "$version" = "nightly build \"v$latest_release_nightly\"" ]; then
    url="https://github.com/randomname32/venus-os_${driver_name}/archive/refs/heads/master.zip"
fi
...
if [ "$driver_name" != "$driver_name_instance" ]; then
    ...
    # change the device_instance from 100 to 100 + driver_instance in the config sample file
